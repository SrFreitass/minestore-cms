   ...
   return (
      <div className='border-1 m-4'>
         <Table className="w-full table-fixed overflow-x-auto text-muted-foreground">
            ...
            <TableHeader>
               <TableRow ref={tableHeadContainerRef as LegacyRef<HTMLTableRowElement>} className='border-x-transparent'>
            ...
            <TableBody>
               {comparisons.map((comparison, index) => {
                  ...
                  return (
                     ...
                     >
                        <TableCell
                           className={`sticky left-0 z-10 flex w-[200px] items-center justify-center ${bgColor} transition-colors group relative`}
                        >
                           <span className="hidden absolute mb-1 justify-self-center group-hover:animate-up group-hover:block rounded-md transition-all bg-royal-sec/80 px-4 text-white" style={{ animationFillMode: 'forwards' }}>
                              {comparison.description}
                           </span>
                           <p className='text-white font-medium w-full'>{comparison.name}</p>
                        </TableCell>
